# PSYC402: Week 12 - Lab activity 2
...
# data, the VoiceID variable). The accoustics data file is tidy and in the long format. The ratings data is in
# the wide format.
...
#
# TASK: Use the gather() function to restructure the ratings data from wide to long and store the resulting table
# as 'ratings_tidy'.

ratings_tidy <- gather(data = ratings, # the data you want to restructure
                       key = participant, # variable name that captures whatever is across the columns you are restructuring, in this case P1 to P28 for the 28 different participants)
                       value = rating, # variable name that captures whatever is in the cells (in this case numbers for ratings)
                       P1:P28) # columns you want to apply th above to
...
joined_wide <- joined %>%
  spread(key = measures, # name of the categorical column to spread
         value = value) # name of the data to spread
...
# Step 10: Power and effect size ------------------------------------------

# TASK: Now let's look at power and effect size. We can calculate the minimum effect size we were able to
# detect given the sample size and design of the study using the pwr.f2.test() function. To do this, we fill
# in all the information we have and set the effect size argument, in this case f2, to NULL.

pwr.f2.test(u = 1, v = 30, f2 = NULL, sig.level = .05, power = .8)      # Calculate minimally detectable effect size

# QUESTION: What is the minimum effect size we were able to detect rounded to two decimal places?
#
# ANSWER: 0.26

# QUESTION: According to Cohen’s guidelines, would this interpreted as a small, medium or large effect?
#
# ANSWER: For f2 the rule of thumb goes f2 > .35 -> large, f2 > .15 -> medium and f2 > .02 small. So: medium

# TASK: Now calculate the observed effect size using the code below:

f2 <- mod_pitch_sum$adj.r.squared/(1 - mod_pitch_sum$adj.r.squared)        # Calculate observed effect size
f2

# QUESTION: Is the observed effect size larger than the minimum effect size we could detect?
#
# ANSWER: Yes, it is .45. This means the study is sufficiently powered.


# Step 11: Writing up the results -----------------------------------------
